function announcements_runAllFormattingFunctions_oneWeekOut(doc) {
  doc = doc || DocumentApp.openById(config.files.announcements.oneWeek)
  announcements_formatFont_oneWeek(doc);
//  announcements_format_removeEmptyParagraphs_oneWeek(doc);
  announcements_format_removeEmptyParagraphs(doc);
}

function announcements_formatFont_oneWeek(doc) {
...
//function announcements_format_removeEmptyParagraphs_oneWeek(doc) { ///rewrite based on the upcoming sunday version
//  doc = doc || DocumentApp.openById(config.files.announcements.oneWeek);
//  var body = doc.getBody();

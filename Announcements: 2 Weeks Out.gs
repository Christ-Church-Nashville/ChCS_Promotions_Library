...
//function announcements_moveThisSunday_to_MasterSundayAnnouncements() {
//  var thisSundayDoc = DocumentApp.openById(config.files.announcements.upcoming);
//  var thisSundayHeading = announcements_getHeading(thisSundayDoc).trim(); //removes any trailing whitespace, to make the search more accurate
//  
...
//function announcements_moveNextSunday_to_thisSunday() {
...
//  }
//  announcements_applyFormattingToHeading(config.files.announcements.upcoming);
//  announcements_applyFormattingBody(config.files.announcements.upcoming);
//  //Draft To Next Sunday
//  announcements_DraftToNextSunday();
//}

//function announcements_DraftToNextSunday() {
...
//  }
//  announcements_applyFormattingToHeading(config.files.announcements.oneWeek);
//  announcements_masterToDraft();
//}

function announcements_applyFormattingBody(id) {
  var style = {};
...
function announcements_applyFormattingToHeading(id) {
  var style = {};
  style[DocumentApp.Attribute.HORIZONTAL_ALIGNMENT] =
  ...
  var firstPara = announcements_getFirstParagraph(doc);
  firstPara.setAttributes(style);
}

function announcements_getHeading(doc) {
...
function announcements_getFirstParagraph(doc) {
  var p = doc.getBody().getParagraphs();
  for (i=0; i<p.length; i++)
    if (p[i].getText() != "")
      return p[i];
...
//function announcements_getNextDraft_OLD() {
...
//function announcements_getNextDraft() {
//  
//  return fDate(
...
//function announcements_masterToDraft() {
//  
...
//  var nextDraftHeaderText = announcements_getNextDraft().trim();
...
function announcements_draft_callFunctions() {
  announcements_draft_removeRowReferences();
  announcements_draft_mergeParagraph();
  //announcements_draft_removeSpaces()
  //announcements_draft_removeBlankPara();
  announcements_draft_formatParagraph();
  announcements_draft_boldBetweenSquareBrackets();
  announcements_draft_highlightStaffSponsorNames();
}

function announcements_draft_removeRowReferences() {
...
function announcements_draft_removeSpaces() {
  var doc = DocumentApp.openById(config.files.announcements.twoWeeks);
  var body = doc.getBody();
  var paras = body.getParagraphs();
  var length = paras.length;
...
function announcements_draft_removeBlankPara() {
  var doc = DocumentApp.openById(config.files.announcements.twoWeeks);
  var body = doc.getBody();
  var paras = body.getParagraphs();
  var length = paras.length;
...
function announcements_draft_mergeParagraph() {
  var doc = DocumentApp.openById(config.files.announcements.twoWeeks);
  var body = doc.getBody();
  var paras = body.getParagraphs();
  var textLocation = {};
...
function announcements_draft_formatParagraph() {
  var doc = DocumentApp.openById(config.files.announcements.twoWeeks);
  var body = doc.getBody();
  var paras = body.getParagraphs();
  //Format Heading
...
function announcements_draft_boldBetweenSquareBrackets() {
  var startTag = "[\[]";
  var endTag = "[\]]";
  var doc = DocumentApp.openById(config.files.announcements.twoWeeks);
  var body = doc.getBody();
...
function announcements_draft_highlightStaffSponsorNames() {
  var doc = DocumentApp.openById(config.files.announcements.twoWeeks);
  var body = doc.getBody();
  var textToHighlight = announcements_checkCCNStaffDataSheet();
  var highlightStyle = {};
...
function announcements_checkCCNStaffDataSheet() {
  var staffArray = [];
  var ccnStaffSheet = SpreadsheetApp.openById(config.files.staffData);
  var lastRow = ccnStaffSheet.getLastRow();
  var staffRows = ccnStaffSheet.getSheetValues(3, 1, lastRow, 2);
  var staffEmails = ccnStaffSheet.getSheetValues(3, 9, lastRow, 1);
  var staffFromSheet = "";
...
function announcements_sendMailFunction() {
  var docSunday = DocumentApp.openById(config.files.announcements.twoWeeks);
  var documentName = docSunday.getName();
  var arrnew = documentName.split("-");
  var paraFirst = arrnew[0];
  ...
  var arr1 = arr[1].split("]");
  var datestr = arr1[0].trim();
  announcements_checkName(datestr);
}

function announcements_driveAPI() { ///rename this to something that makes sense with what it's doing.
  var emailList = "";
...
function announcements_checkName(dateToSend) {
  var emailList = "";
  var staffToEmail = announcements_makestaffMailList();
  
  var comments = ('' + announcements_getCommentsFromDocument()).toUpperCase();
  var emailListArray = [];
  ...
  var array3 = announcements_arrayUnique(emailListArray.concat(commentArryRes));
  var uniqueArray = (announcements_uniq(array3));
  for (em in uniqueArray) {
    emailList += uniqueArray[em] + ",";
  }
  
  announcements_sendDraftMailFinal(emailList, dateToSend);
}

function announcements_uniq(a) {
  var seen = {};
...
function announcements_arrayUnique(array) {
  var a = array.concat();
...
function announcements_makestaffMailList() {///redo this to use getDataRange() then reduce to get needed output
  var ccNashStaffSheet = SpreadsheetApp.openById(config.files.staffData);
  var numRows = ccNashStaffSheet.getLastRow();
  var staffRows = ccNashStaffSheet.getSheetValues(3, 1, numRows, 2);
  var staffEmails = ccNashStaffSheet.getSheetValues(3, 9, numRows, 1);
  var staffFromSheet = [];
...
function announcements_findNameInDraft(staffName) {
  var check = 0;
...
function announcements_removeShortStartDates_OLD() {
  var doc = DocumentApp.openById(config.files.announcements.twoWeeks);
  var pa = doc.getBody().getParagraphs();
  var mdr = /^(\[[^\]\|]+\|[^\]]+\][^01-9]+)([01-9]+\.[01-9]+)([^01-9a-z]+)/gi;
  var mdr2 = '^\\[[^\]\\|]+\\|[^\\]]+\\][^01-9]+([01-9]+\.[01-9]+)[^01-9a-zA-Z]+';
...
function announcements_removeShortStartDates() {
  var doc = DocumentApp.openById(config.files.announcements.twoWeeks);
...
function announcements_modifyDatesInBody() {
  announcements_updateDatePrototype();
  Logger.clear();
  var doc = DocumentApp.openById(config.files.announcements.oneWeek);
  var opa = doc.getBody().getParagraphs(); //opa contains event text
  ...
  var date = announcements_getDateInName(filename);
  //form matching templates
  var templates = announcements_createMatchTemplates(date);
...
function announcements_createMatchTemplates(date) {
  ...
  for (var i = 0; i <= 7; i++) {
    ...
    //{ 
    var template = {};
    var regex = announcements_getDateRegex(dayname, tempDate.getMonth(), tempDate.getDate());
...
function announcements_getDateRegex(dayname, month, date) {
  var regexStr = '';
  var dayRegex = announcements_getDayRegexStr(dayname);
  var monthRegex = announcements_getMonthRegexStr(month);
  var dateRegex = announcements_getDateRegexStr(date);
  //var dateRegex = '(' + date+  ')';
...
function announcements_getDayRegexStr(dayname) {
  switch (dayname) {
    case 'Sunday':
...
function announcements_getMonthRegexStr(month) {
  switch (month) {
    case 0:
...
function announcements_getDateRegexStr(date) {
  var dateRegex = '';
  //(19[ ]{1}|(19)+$)
...
function announcements_getDateInName(name) {
...
//3. Sun, Oct 8
//4. Sun, October 8
function announcements_updateDatePrototype() {

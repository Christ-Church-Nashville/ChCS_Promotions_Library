function announcements_format_master(doc) {
  doc = doc || DocumentApp.openById(config.files.announcements.master);
  announcements_format_doubleSpaceToSingle(doc);
  announcements_format_removeEmptyParagraphs(doc);
  announcements_format_master_fixPageBreaksAndHeadings(doc);
  announcements_format_master_formatEventsFuture(doc);
  announcements_format_master_formatEventsPast(doc);
}

function announcements_format_master_TRIGGERED(){announcements_format_master()}

function announcements_format_master_fixPageBreaksAndHeadings(doc){
  doc = doc || DocumentApp.openById(config.files.announcements.master);
  var body = doc.getBody();//var body = DocumentApp.getActiveDocument().getBody()
...
function announcements_format_removeEmptyParagraphs(doc) {
...
function announcements_format_doubleSpaceToSingle(doc) {
  doc = doc || DocumentApp.getActiveDocument();
  var body = doc.getBody();
  doc.getBody().replaceText("[ ]{2,}", " "); 
}

function announcements_format_master_formatEventsFuture(doc) {
  doc = doc || DocumentApp.openById(config.files. announcements.master);
  var body = doc.getBody();
...
function announcements_format_master_formatEventsPast(doc) {
  doc = doc || DocumentApp.openById(config.files. announcements.master);
  var body = doc.getBody();

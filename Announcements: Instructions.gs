function announcements_showInstructions(html, title){
  var modal = HtmlService.createHtmlOutput(html).setWidth(800).setHeight(600);
  DocumentApp.getUi().showModalDialog(modal, title );
}

function announcements_showInstructions_COPYMEFORNEWINSTRUCTIONSBLOCK(){
  var title = 'Title'
...
";
  ...
  announcements_showInstructions(html, title);
  
}

function announcements_showInstructions_Document(){
...
via the CCN Promotions Library script used by their respective forms.<br>\
<br>\
...
<br></dd>\
\
<dt>Formatting Submenu</dt>\
<dd>Allows the above items to be run singly.<br><br></dd>\
\
<dt>Automation</dt>\
...
function announcements_showInstructions_RecurringContent(){
  var title = 'Recurring Content'
  var html = "<h1>"+title+"</h1> \
\
Recurring Content is automatically added to each new Sunday page as it is created.  \
Recurring Content dirextives are added to the page entitles, aptly enough, [ RECURRING CONTENT ]<br>\
<br>\
<h3>Directives</h3>\

function announcements_rotateContent() {
...
",
  ...
  */
  announcements_moveThisToMaster();
  announcements_moveNextToThis();
...
function announcements_moveThisToMaster(){
  ...
  var thisSundayHeading = announcements_getHeading(thisSundayDoc).trim();//gets the first non-empty paragraph text
...
//    masterBody.getChild(i).setAttributes({FOREGROUND_COLOR:'#ff0000'});
  ...
  var sundayTitles = announcements_getWeekTitles();
  thisSundayDoc.setName(sundayTitles.thisSunday);
  
  announcements_format_master();///FIX  
}

function announcements_moveNextToThis(){
  ...
  announcements_runAllFormattingFunctions_upcomingWeek();
}

function announcements_moveDraftToNext(){
  var sundayTitles = announcements_getWeekTitles();
...
function announcements_moveOldestToArchive(){
...
function announcements_moveSlides() {// delete all content in the destination folder and copy service slides corresponding to the events written in 'This Sunday's Announcements' doc to the destination folder
    ...
    while (filesSource.hasNext()) {
      ...
      if (filename.match(/\.jpg|\.png/)){//only copy image files
        for (var f in fileTitleArray) {
          ...
          if (announcements_compareStrings(filenameSansExt, fileTitleArray[f]) > 0.75) {//check file name against announcement title
  ...
  log('End: announcements_moveSlides()')
}

function announcements_copyContent(fromDoc, toDoc){
...
function announcements_extractSundayTitle(doc){
